...
var checknickName = (rule, value, callback) => {
  if (value) {
    ...
    var reg1 = /^(?!_)(?!.*?_$)[a-zA-Z0-9_\u4e00-\u9fa5]+$/;
    if (value.toString().length > 10) {
      callback(new Error("不能大于10个字符"));
    } else if (!reg.test(value)) {
      callback(new Error("汉字、数字、字母及下划线组合"));
    } else if (!reg1.test(value)) {
      callback(new Error("不能以下划线开头或结尾"));
    } else {
      callback();
...
export let rules = {
  ...
  // 账号
  account: function (isRequired = true) {
    if (isRequired) {
      return [
        { required: true, message: "请输入账号", trigger: "change" },
        { validator: checkuserName, trigger: "change" },
        // { min: 1, max: 10, message: "不能大于10个字符", trigger: "change" },
      ];
    } else {
      return [
        { validator: checkuserName, trigger: "change" },
  ...
  // 关键字
  keyWord() {
    return [{ max: 10, message: "不能大于10个字符", trigger: "change" }];
  },
  // 邮箱验证
  email() {
