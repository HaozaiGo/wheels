  ...
  <div style="height: 100vh;overflow: hidden;">
    ...
    <div class="home-container">
      ...
      <header>
        <div class="header-back">
          <div @click="linkToHome">
            <img class="img" src="@/assets/cage/home_back.png" alt="">
          </div>


          <img class="full-img" @click="showFullScreen" src="@/assets/cage/fullscreen.png" alt="">
          <div style="color: aliceblue;cursor: pointer;" @click="handleHide">
            {{ hideBar ? '展开' : '隐藏' }}
          </div>
        </div>
        ...
        <div class="right">
          <div>
            <p :class="{ 'active': active === 2 }">漏损分析</p>
            <p :class="{ 'active': active === 1 }">设备统计</p>
      ...
      <section ref="sectionRef">
        ...
        <div class="data-info">
          <ul>
            <li v-for="item in cityInfoList" :key="item.id"
              ...
              <div>
                <span :class="[{ 'flop-figure': !isNaN(ls) }, { 'flop-comma': isNaN(ls) }]"
                  v-for="(ls, index) in item.valueArr" :key="item.id + index">
        ...
        <keep-alive>
          <UserDataPreview ref="user" :height="height" :fullscreen="fullscreen" v-if="active === 2" :riseImage="riseImage"
            :declineImage="declineImage"></UserDataPreview>
          <DeviceDataPreview v-if="active === 1" :height="height" :fullscreen="fullscreen" :riseImage="riseImage"
            :declineImage="declineImage"></DeviceDataPreview>
        </keep-alive>
...
import { cityData } from './jsonData'
import Three3D from './threeD.vue'
export default {
  name: 'DataPreview',
  components: { UserDataPreview, DeviceDataPreview, Map, Three3D },
  data() {
    return {
      show3D: true,
      hideBar: false,
      ...
      cityInfoList: [],
      updateTime: '',
      riseImage: require('@/assets/cage/rise.png'),
      declineImage: require('@/assets/cage/decline.png'),
      bubbleData: {
        cityCode: '',
        cityName: '',
        proportion: '',
  ...
  methods: {
    linkToHome() {
      ...
      console.log(route);
      const firstRoute = route[1].path + '/' + route[1].children[0].path + '/' + route[1].children[0].children[0].path
      this.$router.push({ path: firstRoute })
    },
    ...
    handleHide() {
      ...
      this.$children[4].handleHideSideBar();
